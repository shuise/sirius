 ...
 * 识别页面中的脚注/注释标记（[^n] 和 <sup><a href="#fn">），
 * 在原文相应位置生成气泡 tooltip 显示注释内容。
 */
...
function collectFootnoteDefs() {
  ...
  containers.forEach((container) => {
    ...
    items.forEach((item) => {
      const id = item.id || (item.querySelector('a') || {}).name;
      if (id) {
        const text = item.textContent.replace(/^[\d\s.]+/, '').replace(/↩\s*$/, '').trim();
        if (text) defs.set(id, text);
      }
      const backlink = item.querySelector('a[href^="#fnref"]');
      if (backlink) {
  ...
  // Fallback: look for named anchors
  if (defs.size === 0) {
    document.querySelectorAll('a[name^="fn"], a[id^="fn"], li[id^="fn"], p[id^="fn"]').forEach((el) => {
      const id = el.id || el.name || el.getAttribute('id');
      if (id) {
...
function createBubbleForLink(link, noteText) {
  ...
  wrapper.addEventListener('mouseenter', () => { bubble.style.display = 'block'; });
  wrapper.addEventListener('mouseleave', () => { bubble.style.display = 'none'; });
...
function extractAndBubble() {
  ...
  // Style 1: <sup><a href="#fn-*"> (classic footnote links)
  ...
  // Style 2: [^n] pattern in text — needs to look up by index
  processMarkdownAnnotations(defs);
}

function processMarkdownAnnotations(defs) {
  // Map footnote definition IDs that look like "fn-1", "fnref-1", "fn1" to their index
  const defByIndex = new Map();
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode: (node) => {
        ...
        if (!parent || parent.closest('.sirius-annotation-bubble') ||
  ...
  textNodes.forEach((textNode) => {
    ...
    while ((match = annotationRegex.exec(remaining)) !== null) {
      ...
      if (noteText) {
        // Create a clickable sup with bubble
        ...
        sup.addEventListener('mouseenter', () => { bubble.style.display = 'block'; });
        sup.addEventListener('mouseleave', () => { bubble.style.display = 'none'; });
        ...
        // No definition found — render as plain sup
        const sup = document.createElement('sup');
...
export default register({
  ...
  destroy() {
    ...
    document.querySelectorAll('[data-sirius-annotation]').forEach((el) => {
      delete el.dataset.siriusAnnotation;
    });
  },
});
